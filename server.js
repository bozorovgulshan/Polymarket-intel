...
app.get("/api/leaderboard", async (req, res) => {
  try {
    const data = await fetchJson(`${DATA}/leaderboard?window=allTime&limit=30`);
    res.json(data);
...
app.get("/api/positions/:address", async (req, res) => {
  try {
    const data = await fetchJson(`${DATA}/positions?user=${req.params.address}&sizeThreshold=0&limit=50`);
    res.json(data);
  } catch (e) { res.status(502).json({ error: e.message }); }
});
